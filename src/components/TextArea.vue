<template>
  <div
      ...
      @keydown.prevent.space="keySpace"
      @keydown.prevent="$event.key === 'Backspace' ? keyBackspace() : keyDown($event)"
      @blur.prevent="onBlur"
  ...
  >
    ...
    <span
        class="absolute text-yellow-600 transition-transform top-0 -left-2 pulse font-semibold"
        ref="textCursor"
...
export default defineComponent({
  ...
  watch: {
    '$store.state.text' : function(newVal : string[][]) {
  ...
  computed: {
    ...
    cursorPosition() {
      ...
      return `calc(${wordOffset.left}px + ${charOffset * 0.6}em), ${wordOffset.top}px`;
    },
    stats() {
